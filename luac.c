...
static int doargs(int argc, char **argv : itype(_Array_ptr<char*>))
{
 int i;
 int version=0;
...
static void PrintConstant(const Proto *f : itype(_Ptr<const Proto>), int i)
{
 ...
 switch (ttype(o))
 {
  ...
  case LUA_TNUMFLT:
	{
	char buff[100];
	sprintf(buff,((const char *)LUA_NUMBER_FMT),fltvalue(o));
	printf(((const char *)"%s"),buff);
	if (buff[strspn(buff,"-0123456789")]=='\0') printf(((const char *)".0"));
	break;
	}
  case LUA_TNUMINT:
	printf(((const char *)LUA_INTEGER_FMT),ivalue(o));
	break;
  case LUA_TSHRSTR: case LUA_TLNGSTR:
	PrintString(((const TString *)tsvalue(o)));
	break;
...
static void PrintCode(const Proto *f : itype(_Ptr<const Proto>))
{
